#property strict
#property description "ScalpGuru V8 - Aggressive 3-Month Funded Pass Edition"
#property description "Enhanced sells with Stochastic, trend filter, optimized for $200+/month"
...
//--- Trading Configuration (V8: Aggressive defaults for 3-month pass)
input group "=== Trading Configuration ==="
input int MagicNumber = 15138;              // Magic Number (V8)
...
input double RiskPerTradePercent = 1.0;     // Risk per trade % (V8: 1% for faster growth)
...
input int MaxTradesPerDay = 3;              // Max trades per day (V8: 3 for more opportunities)
input bool EnableMaxTradesPerDay = true;    // Enable/disable max trades per day limit

//--- Funded Account Protection (V8 - Aggressive but safe limits)
input group "=== Funded Account Protection ==="
input bool EnableFundedMode = true;          // Enable funded account protection
input double DailyLossLimitPercent = 2.9;    // Max daily loss % (V8: closer to 3% limit)
input double MaxDrawdownPercent = 5.8;       // Max overall drawdown % (V8: closer to 6% limit)
...
input int RSI_Oversold = 32;                // RSI Oversold level for buys (V8: relaxed from 30)
input int RSI_Overbought = 68;              // RSI Overbought level for sells (V8: relaxed from 70)
...
void CreateInfoPanel()
{
   ...
   // Create title
   CreateLabel("SG_Title", "═══ ScalpGuru V8 ═══", panelX + 25, panelY + 10, clrGold, 10, "Arial Bold");
