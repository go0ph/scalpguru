#property strict
#property description "ScalpGuru V9 - Data-Driven Gold Trading Edition"
#property description "Enhanced with XAUUSD historical data analysis, volatility-adjusted risk, optimal session filtering"
...
//--- Trading Configuration (V9: Data-driven optimization based on 20+ years XAUUSD data)
input group "=== Trading Configuration ==="
input int MagicNumber = 15139;              // Magic Number (V9)
...
input double RiskPerTradePercent = 1.0;     // Risk per trade % (V9: dynamically adjusted by volatility)
...
input int MaxTradesPerDay = 4;              // Max trades per day (V9: 4 for optimal opportunities)
input bool EnableMaxTradesPerDay = true;    // Enable/disable max trades per day limit

//--- Funded Account Protection (V9 - Enhanced safety with data-driven limits)
input group "=== Funded Account Protection ==="
input bool EnableFundedMode = true;          // Enable funded account protection
input double DailyLossLimitPercent = 2.5;    // Max daily loss % (V9: safer than V8's 2.9%)
input double MaxDrawdownPercent = 5.5;       // Max overall drawdown % (V9: well within 6% limit)
...
//--- Entry Filters (V8: Slightly relaxed for more trades)
input group "=== Entry Filters ==="
input bool EnableMomentumFilter = true;     // Enable RSI momentum filter
input int RSIPeriod = 14;                   // RSI Period
input int RSI_Oversold = 30;                // RSI Oversold level for buys (V9: optimized from data)
input int RSI_Overbought = 70;              // RSI Overbought level for sells (V9: optimized from data)
...
input bool RequireKeltnerRetest = false;    // V8: Disabled for more opportunities
...
//--- V9 NEW: Data-Driven Enhancements (Based on 20 years XAUUSD historical data)
...
void CreateInfoPanel()
{
   ...
   // Create title
   CreateLabel("SG_Title", "═══ ScalpGuru V9 ═══", panelX + 25, panelY + 10, clrGold, 10, "Arial Bold");
   ...
   CreateLabel("SG_VolRegime", "Vol: Normal", panelX + 10, panelY + 195, clrCyan, 9, "Arial");  // V9 NEW
   CreateLabel("SG_RiskAdj", "Risk: 1.0x", panelX + 10, panelY + 215, clrCyan, 9, "Arial");     // V9 NEW
}
...
void UpdateInfoPanel()
{
   ...
   // V9: Update risk multiplier display
...
void OpenBuyTrade()
{
   ...
   if(trade.Buy(lotSize, _Symbol, price, sl, takeProfit, "ScalpGuru V8 Buy"))
   {
      inTrade = true;
      tradesToday++;
      trailingActive = false;
...
void OpenSellTrade()
{
   ...
   if(trade.Sell(lotSize, _Symbol, price, sl, takeProfit, "ScalpGuru V8 Sell"))
   {
      inTrade = true;
      tradesToday++;
      trailingActive = false;
...
double GetVolatilityAdjustedSL()
{
   ...
   if(atrValue > 14.74)
   {
      return SL_ATRMultiplier * HighVolStopMultiplier;  // 1.3 * 0.9 = 1.17
   }
...
double CalculateLotSize(double price)
{
   ...
   {
      // V9: Apply volatility-adjusted risk
      double volMultiplier = GetVolatilityMultiplier();
      double adjustedRiskPercent = RiskPerTradePercent * volMultiplier;
